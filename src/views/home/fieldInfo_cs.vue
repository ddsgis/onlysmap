...
export default {
  ...
  methods: {
    ...
    pulleyRoll(top, distance) {
      ...
      /* 向上滚动*/
        var small_interval1 = (distance - top) / 50;
        var k = 0;
        var timer1 = setInterval(() => {
          k++;
          console.log(distance -= small_interval1);
          document.documentElement.scrollTop = distance;
          if (k === 50) {
    ...
    getFloorDistance() {
      ...
      const floorThree = document.getElementsByClassName('floor_three')[0].offsetTop;
      const floorFour = document.getElementsByClassName('floor_four')[0].offsetTop;
      const floorFive = document.getElementsByClassName('floor_five')[0].offsetTop;
